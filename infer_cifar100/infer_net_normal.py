...
class Privacy_Trainer:
    ...
    def prepare_Dataset(self, train_set=None, val_set=None, test_set=None, **kwargs):
        ...
        if train_set is not None:
            ...
            elif train_set == Coarse_Cifar100:
                self.train_set = train_set(root=self.path["train_path"], train=True,
        ...
        if val_set is not None:
            ...
            elif val_set == Coarse_Cifar100:
                self.val_set = val_set(self.path["val_path"],transform=self.val_transform, train=False, download=False)
    ...
    def load_Latest_Epoch(self):
        ...
        self.model_part1.load_state_dict(torch.load(model_part1_path))
        self.model_part2.load_state_dict(torch.load(model_part2_path))
        self.model_part3.load_state_dict(torch.load(model_part3_path))
...
def main():
    ...
    parser.add_argument("--epoch-num", default=200, type=int)
    parser.add_argument("--num-classes", default=20, type=int)
    parser.add_argument("--num-layers", default=56, type=int)
    ...
    parser.add_argument("--encrypt-location", default="a", type=str)
    parser.add_argument("--result-path", default="resnet56_cifar100_infer", type=str)
    parser.add_argument("--sub-result-path", default="net_normal_0.1_log", type=str)
    ...
    parser.add_argument("--G-lr", default=1e-5, type=float)
    parser.add_argument("--D-lr", default=1e-5, type=float)
    parser.add_argument("--GAN-step", default=25, type=int)
    ...
    trainer.prepare_Model(Get_Model_ResNet, optim.SGD, optim.RMSprop, nn.CrossEntropyLoss)
    ...
    trainer.prepare_Dataset(train_set=Coarse_Cifar100, val_set=Coarse_Cifar100,
                            train_subpath=os.path.join("data", "Coarse_cifar"),
                            val_subpath=os.path.join("data", "Coarse_cifar"),
