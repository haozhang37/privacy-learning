...
class eval_baseline_2:
    ...
    def prepare_dataset(self):
        self.train_set = datasets.ImageFolder(self.path["data_path"] + "/train", transform=self.train_transform)
        self.train_loader = torch.utils.data.DataLoader(self.train_set,batch_size=self.bs, shuffle=self.train_shuffle)

        self.val_set = datasets.ImageFolder(self.path["data_path"] + "/test", transform=self.val_transform)
...
def main():
    ...
    parser.add_argument("--train-id",default=2,type=int)
    parser.add_argument("--num-classes", default=100, type=int)
    parser.add_argument("--load-epoch", default=0, type=int)

    parser.add_argument("--result-path", default="resnet56_cifar100_infer/eval_best_sample/", type=str)
    parser.add_argument("--data-path", default="data/infer_cifar100/best_decode")
