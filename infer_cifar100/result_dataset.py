...
from coarse_dataset import Coarse_Cifar100
...
#     torch.a(samples, "./data/CelebA_theta_large.bin")
...
def get_models(args):
    model_part1, model_part2, model_part3 = Get_Model_ResNet(args.device, args.is_encrypt, args.dim, args.double_filter,
...
def get_paths(args):
    if args.decoder_type == 1:
        ...
        sub_folder = "net_normal_0.1_log"
        decoder_folder = args.is_unet + "decoder_normal_nonencrypt"
    elif args.decoder_type == 4:
        sub_folder = "net_normal_0.1_log" # original: net_privacy_GAN
...
def get_dataset(args):
    train_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=args.cifar100_mean, std=args.cifar100_std)
    ])
    val_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=args.cifar100_mean, std=args.cifar100_std)
    ])
    train_dataset = torchvision.datasets.CIFAR100(args.data_path, train=True, transform=train_transform, download=True)
    val_dataset = torchvision.datasets.CIFAR100(args.data_path, train=False, transform=val_transform, download=True)
...
def get_results(args):
    ...
    train_dataset, val_dataset, train_dataloader, val_dataloader = get_dataset(args)
    ...
    img_dataset = "./data/infer_cifar100/best_decode_dim%d/" % args.dim
    fea_dataset = "./data/infer_cifar100/best_feature_dim%d/" % args.dim
    ...
    with torch.no_grad():
        for i, (img, lbl) in enumerate(train_dataloader):
            ...
            for j in range(batch_use * args.dim):
                ...
                class_name = train_dataset.classes[lbl[j]]
                feat_train_folder = feat_train + class_name
                if not os.path.exists(feat_train_folder):
                    os.mkdir(feat_train_folder)
                best_train_img_folder = best_train_img + class_name
                if not os.path.exists(best_train_img_folder):
                    os.mkdir(best_train_img_folder)
                feat_name = feat_train_folder + "/" + str(number).zfill(5) + ".bin"
                img_name = best_train_img_folder + "/" + str(number).zfill(5) + ".jpg"
        ...
        for i, (img, lbl) in enumerate(val_dataloader):
            ...
            for j in range(batch_use * args.dim):
                number = i * args.batch_size + j + 1
                class_name = train_dataset.classes[lbl[j]]
                feat_test_folder = feat_test + class_name
                if not os.path.exists(feat_test_folder):
                    os.mkdir(feat_test_folder)
                best_test_img_folder = best_test_img + class_name
                if not os.path.exists(best_test_img_folder):
                    os.mkdir(best_test_img_folder)
                feat_name = feat_test_folder + "/" + str(number).zfill(5) + ".bin"
                img_name = best_test_img_folder + "/" + str(number).zfill(5) + ".jpg"
                torch.save(feature[j].data.cpu(), feat_name)
                img = generateImg(decoder_out[j].cpu())
                img.save(img_name)
...
def main():
    ...
    parser.add_argument("--data-path", default="./data/Coarse_cifar", type=str)
    # parser.add_argument("--batch-size", default=100, type=int)
    parser.add_argument("--net-layer-num", default=56, type=int)
    parser.add_argument("--net-class-num", default=20, type=int)
    parser.add_argument("--sample-num", default=1000, type=int)
    parser.add_argument("--sample-num-for-all-train-sample", default=100000, type=int)# generate 100000 rotation matrices, since CIFAR-100 only has 50000 training images, 100000 is enough
    parser.add_argument("--visual-num", default=30, type=int)
    parser.add_argument("--encrypt-position", default="a", type=str)
    parser.add_argument("--if-alexnet", default=0, type=int)
    parser.add_argument("--num-input-channels", default=32, type=int)
    parser.add_argument("--num-block", default=6, type=int)
    parser.add_argument("--num-block-upsample", default=1, type=int)
    ...
    parser.add_argument("--load-net-epoch", default=199, type=int)
    parser.add_argument("--load-decoder-epoch", default=29, type=int)
    # parameter for GAN
    parser.add_argument("--D-boardsize", default=16, type=int)
    parser.add_argument("--D-channel", default=32, type=int)
    parser.add_argument("--D-block", default=1, type=int)
    parser.add_argument("--D-kernel", default=3, type=int)
    parser.add_argument("--G-boardsize", default=16, type=int)
    parser.add_argument("--G-channel", default=32, type=int)
    ...
    args.cifar100_mean = [0.507, 0.487, 0.441] # mean for cifar100
    args.cifar100_std = [0.267, 0.256, 0.276] # std for cifar100
    # args.result_path = "./result/resnet56_cifar100_infer"  # result_path
