...
class Decoder:
    def __init__(self, args):
        ...
        if self.decoder_type == 1:
            ...
            self.model_path = os.path.join(self.result_path, 'net_noloss')
            self.decoder_path = os.path.join(self.result_path, 'unet_decoder_noloss')
        elif self.decoder_type == 3:
            self.model_path = os.path.join(self.result_path, 'net_normal_0.1_log')
            self.decoder_path = os.path.join(self.result_path, 'unet_decoder_normal_nonencrypt')
        elif self.decoder_type == 4:
            self.model_path = os.path.join(self.result_path, 'net_normal_0.1_log')
            self.decoder_path = os.path.join(self.result_path, 'unet_decoder_normal')
        else:
        ...
        if not os.path.exists(self.decoder_path):
            os.makedirs(self.decoder_path)
        if not os.path.exists(self.sub_decoder_path):
            os.makedirs(self.sub_decoder_path)
        ...
        #dataset paramunet_decoder_trainer.py
        self.cifar10_mean = [0.507, 0.487, 0.441]
        self.cifar10_std = [0.267, 0.256, 0.276]
    ...
    def prepare_Model(self):
        ...
        #resnet
        self.resnet_part1, self.resnet_part2, self.resnet_part3 = Get_Model_ResNet(gpu_id=self.device, IsEncrypt=self.IsEncrypt, dim=self.dim,
    ...
    def prepare_Dataset(self):
        normalize = transforms.Normalize(mean=self.cifar10_mean, std=self.cifar10_std)
        training_set = Coarse_Cifar100(root=self.data_path, train=True,
        ...
        val_set = Coarse_Cifar100(root=self.data_path, train=False, transform=transforms.Compose([transforms.ToTensor(),
                                                                                              normalize]),
                                   download=False)
    ...
    def get_privacy_key(self, bs):  # here bs was not divided by dim yet
        ...
        while key_num < bs // self.dim:
            ...
            if torch.det(temp_tensor) > 0:
                ...
                key_num = key_num + 1
        return rot_mat.to(self.device)  # rqh 0525 add
    ...
    def train_model(self):
        ...
        for i, (image, cifar10_label) in enumerate(self.train_loader):
            ...
            cifar10_label = cifar10_label[:batch_use].to(self.device)

            rot_mat = self.get_privacy_key(batch_use)
            if self.IsEncrypt == True:
                if self.IsNoise == True:
                    noise = get_noise(image,mean=self.cifar10_mean,std=self.cifar10_std)
            ...
            for channel_id in range(3):
                image[:, channel_id, :, :] = image[:, channel_id, :, :] * self.cifar10_std[channel_id]
                image[:, channel_id, :, :] = image[:, channel_id, :, :] + self.cifar10_mean[channel_id]
    ...
    def val_model(self):
        ...
        with torch.no_grad():
            for i, (image, cifar10_label) in enumerate(self.val_loader):
                image_size = image.size()
                batch_use = image.size(0)
                batch_use = batch_use // self.dim * self.dim
                if batch_use == 0:
            ...
            return Origins, Decoder, val_loss.avg
    ...
    def saveImg(self,epoch, Origin_imgs, Decoder_img):
        ...
        if os.path.exists(visual_path) == False:
            os.mkdir(visual_path)
        for i in range(len(Origin_imgs[0])):
    ...
    def work(self):
        ...
        logs = torch.logspace(start=st, end=st - 2, steps=self.num_epoch)
...
def main():
    ...
    parser.add_argument("--decoder-type", default=4, type=int, choices=[1, 2, 3, 4],
    ...
    # args.result_path = "./result/resnet%d_cifar%d" % (args.net_layer_num, args.net_class_num)  # result_path
