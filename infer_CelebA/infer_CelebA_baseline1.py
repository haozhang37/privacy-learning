...
class Origin_Trainer:
    ...
    def prepare_Model(self, model, optimizer=None, loss_fn=None, scheduler=None):
        ...
        self.list = [[] for _ in range(6)]
        # result list: [train loss, train error, val loss, val error, best_loss, best_error,]
    ...
    def prepare_Dataset(self, train_set=None, val_set=None, test_set=None, **kwargs):
        ...
        if val_set is not None:
            ...
            if val_set == datasets.CIFAR10 or val_set == datasets.CIFAR100:
                ...
                self.val_set2 = val_set('list_attr_celeba.txt', dst_path=kwargs["val_subpath2"], training=False)
            self.val_loader = torch.utils.data.DataLoader(self.val_set, batch_size=self.bs,
                                                          shuffle=self.val_shuffle)
            self.val_best_loader = torch.utils.data.DataLoader(self.val_set2, batch_size=self.bs,
    ...
    def val_Model(self):
        ...
        with torch.no_grad():
            ...
            for i, (img, lbl) in enumerate(self.val_best_loader):
                img = img.to(self.device)
                lbl = lbl.to(self.device)
                batch_use = img.size(0)
                batch_use = batch_use // self.dim * self.dim
                ...
                BestLoss += loss.cpu().item()
                BestError += error

                # Print the result if you want to
                print("[val] batch: %d, loss: %.5f, error: %.5f" % (i, BestLoss / (i + 1), BestError / (i + 1)))
                # if i>=397: #todo: check
                #     break
            BestLoss, BestError = BestLoss / (i + 1), BestError / (i + 1)
        return Loss, Error, BestLoss, BestError
    ...
    def test_Model(self):
        ...
        with torch.no_grad():
            ...
            for i, (img, lbl) in enumerate(self.val_best_loader):
                ...
                # Print the result if you want to
                print("[test] batch: %d, loss: %.5f, error: %.5f" % (i, BestLoss / (i + 1), BestError / (i + 1)))
                # if i>=397: #todo:check
                #     break
            BestLoss, BestError = BestLoss / (i + 1), BestError / (i + 1)
        return BestLoss, BestError
    ...
    def draw_Figure(self):
        ...
        y4 = np.array(self.list[4])
        y5 = np.array(self.list[5])
    ...
    def work(self):
        ...
        for epoch in range(self.start_epoch, self.epoch_num):
            ...
            loss, error, best_loss, best_error = self.val_Model()
            self.list[2].append(loss)
            self.list[3].append(error)
            self.list[4].append(best_loss)
            self.list[5].append(best_error)

            if epoch % 10 == 9 or epoch == self.epoch_num - 1:
                self.save_Latest_Epoch(epoch)
            self.draw_Figure()

        test_loss, test_error = self.test_Model()
        print(test_error)
...
def main():
    ...
    parser.add_argument("--sub-result-path", default="eval_baseline_1", type=str)
    ...
    trainer.prepare_Model(models.resnet50, optim.SGD, nn.BCEWithLogitsLoss)
    ...
    trainer.prepare_Dataset(train_set=ourDataset, val_set=ourDataset,
                            ...
                            val_subpath2="/data/CelebA_best_img_dim%d" % args.dim,
