...
class Privacy_Trainer:
    ...
    def prepare_Model(self, model, optimizer=None, GAN_optimizer=None, loss_fn=None, scheduler=None):
        ...
        alexnet = torchvision.models.alexnet(pretrained=False, num_classes=30).to(self.device)
        self.model_part1.copy_params_from_alexnet(alexnet)
        self.model_part2.copy_params_from_alexnet(alexnet, self.is_encrypt)
        self.model_part3.copy_params_from_alexnet(alexnet)
    ...
    def load_Latest_Epoch(self):
        ...
        self.model_part1.load_state_dict(torch.load(model_part1_path, map_location=torch.device("cpu")).state_dict())
        self.model_part2.load_state_dict(torch.load(model_part2_path, map_location=torch.device("cpu")).state_dict())
        self.model_part3.load_state_dict(torch.load(model_part3_path, map_location=torch.device("cpu")).state_dict())
        self.G.load_state_dict(torch.load(G_path, map_location=torch.device("cpu")).state_dict())
        self.D.load_state_dict(torch.load(D_path, map_location=torch.device("cpu")).state_dict())
    ...
    def train_Model(self):
        ...
        for i, (img, lbl) in enumerate(self.train_loader):
            ...
            if batch_use < 1:
                continue
            img = img[:batch_use]
            lbl = lbl[0:int(batch_use)]
            lbl = lbl[:,self.ClassAttr].to(self.device)
...
def main():
    ...
    parser.add_argument("--sub-result-path", default="net_normal", type=str)
    ...
    parser.add_argument("--G-lr", default=5e-3, type=float)
    ...
    parser.add_argument("--train-id", default="3", type=str)
    ...
    args.sub_result_path = args.sub_result_path + args.train_id
    ...
    args.ClassAttr = [0, 1, 4, 5, 7, 8, 9, 10, 11, 12,
                 13, 14, 16, 17, 18, 20, 21, 22, 24, 26,
