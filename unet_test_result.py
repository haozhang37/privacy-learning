...
#     torch.save(samples, "./data/theta.bin")


# def get_privacy_key(bs):
#     theta = torch.rand(bs // 3) * 2 * math.pi
#     o1 = torch.randn(bs // 3)
#     o2 = torch.randn(bs // 3)
#     o3 = torch.randn(bs // 3)
#     norm = torch.sqrt(o1 * o1 + o2 * o2 + o3 * o3)
#     o1, o2, o3 = o1 / norm, o2 / norm, o3 / norm
#     return theta, o1, o2, o3


def generate_best_sample(args, feature, D): #todo: understand
    ...
    with torch.no_grad():
        for i in range(rot_mat.size(0)):
            rot_mat_use = rot_mat[i].expand(batch_use,args.dim,args.dim).clone() # rqh 0528 add clone()
...
def get_models(args):
    if "lenet" in args.result_path:
        model_part1, model_part2, model_part3 = Get_Model_Lenet(args.device, args.is_encrypt, args.dim, args.double_filter,
...
def get_paths(args):
    if args.decoder_type == 1:
        ...
        decoder_folder = args.is_unet + "decoder_origin"
    elif args.decoder_type == 2:
        sub_folder = "net_addlayer"
        decoder_folder = args.is_unet + "decoder_addlayer"
    ...
    if args.decoder_type < 5:
        ...
        visual_path = "%s/visual_result/%s/%s/" % (args.result_path, "best_sample", train_id)
...
def get_dataset(args):
    ...
    dataset = torchvision.datasets.CIFAR10(args.data_path, train=False, transform=val_transform, download=True)
        ...
        for i, (img, lbl) in enumerate(dataloader):
            ...
            if i <= args.visual_num or args.decoder_type == 5 or args.decoder_type == 4:
                for channel_id in range(3):
                    img[:, channel_id, :, :] = img[:, channel_id, :, :] * args.cifar10_std[channel_id]
                    img[:, channel_id, :, :] = img[:, channel_id, :, :] + args.cifar10_mean[channel_id]
...
def main():
    ...
    parser.add_argument("--net-class-num", default=10, type=int)
    parser.add_argument("--sample-num", default=1000, type=int)
    parser.add_argument("--sample-num-for-all-test-sample", default=20000, type=int) # generate 20000 rotation matrices, since CIFAR-10 only has 10000 test images, 20000 is enough
    ...
    parser.add_argument("--result-path", default="lenet_cifar10", type=str)
    parser.add_argument("--decoder-type", default=4, type=int, choices=[1, 2, 3, 4, 5],
    ...
    parser.add_argument("--load-net-epoch", default=49, type=int)
    ...
    parser.add_argument("--is-unet", default="unet_", type=str)
