...
class alexnet_part2(nn.Module):
    def __init__(self, isEncrypt, dim):
        ...
        self.dim = dim
        self.pool1 = our_mpool(3, 2)
        self.conv2 = nn.Conv2d(64, 192, kernel_size=5, padding=2, bias=not isEncrypt)
    ...
    def forward(self, x,):
        x = self.pool1(x, self.IsEncrypt, self.dim)
    ...
    def copy_params_from_alexnet(self, alexnet, isEncrypt):
        ...
        for l1, l2 in zip(alexnet.features[2:], features):
            if isinstance(l1, nn.Conv2d) and isinstance(l2, nn.Conv2d):
                assert l1.weight.size() == l2.weight.size()
                l2.weight.data = l1.weight.data
                if not isEncrypt:
